...
function moo(message, options) {
  ...
  /** @type {import("./mode").CowFace} */
  var face = {};

  // Find face by mode
  if (typeof opts.mode === "string") {
    face = mode.modeFace(opts.mode);
  }

  // Custom face
  if (typeof opts.eyes === "string" && face.eyes === undefined) {
    face.eyes = opts.eyes;
  }
  if (typeof opts.tongue === "string" && face.tongue === undefined) {
    face.tongue = opts.tongue;
  }
  ...
  return box.perform(action, message, opts.wrap) + cow.render(act, face.eyes, face.tongue);
}
