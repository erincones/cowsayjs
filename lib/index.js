...
/**
 *
 * @param {ReadonlyArray<import("../cows").Position> | undefined} arr
 * @returns
 */
function validatePositionArray(arr) {
  ...
  return arr.every(function(pos) {
    return Array.isArray(pos) && pos.length === 2 && typeof pos[0] === "number" && typeof[1] === "number";
  });
}
...
function moo(message, options) {
  ...
  switch (typeof opts.cow) {
    ...
    case "object":
      ...
      valid = valid && (opts.cow.defTongue === undefined || typeof opts.cow.defTongue === "string");
      valid = valid && validatePositionArray(opts.cow.actionPos);
      valid = valid && validatePositionArray(opts.cow.eyesPos);
      valid = valid && validatePositionArray(opts.cow.tonguePos);

      cow = valid ? opts.cow : cows.corral[0];
      break;
    default:
      cow = cows.corral[0];
  }
